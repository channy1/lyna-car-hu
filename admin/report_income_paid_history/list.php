            ...
            <div class="card">
				...
				<div class="card-body">
					<table id="example2" class="table table-bordered table-hover">
						...
						<tbody>
							<?php
								$query="SELECT * FROM tbl_partner_income_paid_history INNER JOIN tbl_users ON tbl_users.user_id = tbl_partner_income_paid_history.partner_id ORDER BY start_date DESC LIMIT 150 ";
								$result = $connect->query($query);
								$i=1;
								if ($result->num_rows > 0) {
								while($row = $result->fetch_assoc()){
									$delete_url="dalete.php?delete_id=".$row['p_id'];
							?>
								<tr>
									<td><?php echo $i++; ?></td>
									<td><?php $start_date = strtotime($row['start_date']); echo date('d M y', $start_date); ?></td>
									<td><?php echo $row['user_introducer_id'];?></td>
									...
									<td>
										...
										<a href="#" onclick="delete_data('<?php echo $delete_url;?>')" class="btn btn-xs btn-danger" title="delete"><i class="fa fa-remove"></i>Delete</a>
									</td>
								</tr>
							<?php } } ?> 
						</tbody>
